...
1. POST request: 
    ...
    When there is an error, returns a message in JSON format.
...
GET request:
    1. /reviews/type/{mediaType} - Get all reviews of a specific media type
       ...
       /reviews/type/SHOW
       /reviews/type/MUSIC
   2. /users/id/{id} - Get ResponseEntity object for provided id.
   3. /users/email/{email} - Get ResponseEntity object for provided email.
   4. /users/name/{name} - Get ResponseEntity object for provided name.
   
   Response Entity:
       An object that contains 2 values:
           For error: Contains an error message and a response with the value 404. Can extract status code using response.code().
           For success: Contains a user and a response with the value 200. Can extract user data using response.body().
...
4. DELETE a record - 
 DELETE request:
    1. /users/id/{id} - Delete the User with id provided in the URL
    2. /users/email/{emailId} - Delete the User with id provided in the URL.
    3. /reviews/{id} - Delete the Review with id provided in the URL
